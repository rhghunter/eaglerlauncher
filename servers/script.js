...
const servers = [
    {
        ...
        description: "The main Eaglercraft server with multiple game modes including Survival, Creative, and Skyblock. Features a friendly community, regular events, and a stable economy system. Perfect for both new and experienced players looking for a reliable Minecraft experience in their browser."
    },
    {
        name: "VanillaMC",
        ...
        description: "VanillaMC is a popular Factions server that has a competitive community with a balanced economy and enjoyable pvp. Join factions, build bases, and conquer your enemies in this competitive Minecraft environment."
    },
    {
        name: "Voidsent MC",
        ...
        description: "Voidsent MC offers a variety of gameplay experiences from vanilla survival to competitive PvP. With multiple game modes and a dedicated community, there's something for every type of Minecraft player."
    },
    {
        name: "AdderallMC",
        ip: "wss://adderall.ir",
        ...
        description: "Survival runs on 1.12.2, OneBlock is on 1.20.1. We're a fairly new server (about 2 months old) and always open to suggestions. Hit us up on Discord if you wanna share ideas. Cross-platform, chill staff, friendly community, and some custom stuff to keep it fresh. Come through and have fun."
    },
    {
        name: "Mytheria",
    ...
    },
    {
        ...
        description: "Looking for a Minecraft server with no pay-to-win ⚖️, a respectful community 🤝, and awesome moderation 🛡️? Look no further! Time Legacy is a cross-platform server supporting both Java and Eaglercraft — and we’re excited to have you join us! Let’s build something amazing together!"
    },
    {
...
const popupCopyIP = document.getElementById('popupCopyIP');
const popupTestConnection = document.getElementById('popupTestConnection');

let currentFilter = 'all';
let serverStatuses = {};
let autoRefreshInterval;
let currentTestingServer = null;
let currentPopupServer = null;
...
function initServerList() {
    ...
    servers.forEach(server => {
        ...
        serverCard.innerHTML = `
            ...
            <div class="server-ip">
                <div class="ip-address">${server.ip.replace('wss://', '')}</div>
            </div>
        `;
        
        // Add click event to show popup
        serverCard.addEventListener('click', () => {
            showServerPopup(server);
        });
...
function showServerPopup(server) {
    currentPopupServer = server;
    
    // Update popup content
    document.getElementById('popupServerName').textContent = server.name;
    document.getElementById('popupVersion').textContent = server.version;
    document.getElementById('popupGamemodes').textContent = server.gamemodes.join(', ');
    document.getElementById('popupDescription').textContent = server.description;
    ...
    if (status) {
        ...
        document.getElementById('popupServerStatus').className = 'server-status status-checking';
        document.getElementById('popupStatusText').textContent = 'Checking...';
        document.getElementById('popupPlayerCount').textContent = '-/-';
    }
...
function updatePopupStatus(status) {
    const statusElement = document.getElementById('popupServerStatus');
    const statusText = document.getElementById('popupStatusText');
    const playerCount = document.getElementById('popupPlayerCount');
    
    if (status.online) {
        statusElement.className = 'server-status status-online';
        statusText.textContent = 'Online';
        playerCount.textContent = `${status.players.online}/${status.players.max}`;
    } else {
    ...
    eaglerFrame.srcdoc = `
        ...
        <body>
            <div class="status" id="status">
                ...
                <p id="message">Attempting to connect to ${serverIp.replace('wss://', '')}</p>
                <div id="progress"></div>
            </div>
            <script>
                let connected = false;
                ...
                try {
                    ...
                    ws.onopen = function() {
                        ...
                        document.getElementById('progress').innerHTML = '<div style="color: green;">✓ Server is online</div>';
                    ...
                    ws.onerror = function() {
                        ...
                        document.getElementById('progress').innerHTML = '<div style="color: red;">✗ Server is offline</div>';
                    ...
                    setTimeout(() => {
                        if (!connected) {
                            ...
                            document.getElementById('progress').innerHTML = '<div style="color: red;">✗ Server is offline (timeout)</div>';
                    ...
                    document.getElementById('progress').innerHTML = '<div style="color: red;">✗ Connection failed</div>';
...
window.addEventListener('message', function(event) {
    if (event.data.type === 'eaglerTestResult') {
        ...
        if (serverCard) {
            ...
            // Update popup if it's open for this server
            if (currentPopupServer && currentPopupServer.ip === server) {
                updatePopupStatus(status);
            }
        }
        ...
        testingInfo.innerHTML = `
            ...
            <div class="debug-info">Server: ${server.replace('wss://', '')}</div>
        `;
    }
});
...
async function checkServerStatus(serverIp) {
    let status = { online: false, players: { online: 150, max: 100 }, version: 'Unknown', method: 'WebSocket Test' };
...
function updateServerCard(card, status) {
    ...
    if (status.online) {
        ...
        const percentage = status.players.max > 0 ? (status.players.online / status.players.max) * 100 : Infinity;
    ...
    // Update popup if it's open for this server
    if (currentPopupServer && currentPopupServer.ip === card.dataset.ip) {
        updatePopupStatus(status);
    }
}
...
closePopup.addEventListener('click', closeServerPopup);

popupCopyIP.addEventListener('click', () => {
    if (currentPopupServer) {
        copyIP(currentPopupServer.ip.replace('wss://', ''));
    }
});

popupTestConnection.addEventListener('click', () => {
    if (currentPopupServer) {
        testServerWithEaglercraft(currentPopupServer.ip);
        closeServerPopup();
    }
});

// Close popup when clicking outside content
serverPopup.addEventListener('click', function(e) {
    if (e.target === this) {
        closeServerPopup();
    }
});
